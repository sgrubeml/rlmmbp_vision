...
class IsaacEnvMushroom(Environment):
    ...
    def set_task(self, task, backend="torch", sim_params=None, init_sim=True) -> None:
        ...
        self._world = World(
            stage_units_in_meters=1.0, physics_dt=sim_params["dt"], rendering_dt=sim_params["dt"], backend=backend, sim_params=sim_params, device=self._device, set_defaults=False
        )
    ...
    def reset(self, state=None):
        ...
        self._task.reset()
        # trigger task reset_idx to get current point cloud after stepping from get_observations
        reset_env_ids = self._task.reset_buf.nonzero(as_tuple=False).squeeze(-1)
        self._task.reset_idx(reset_env_ids, self._world.scene)
