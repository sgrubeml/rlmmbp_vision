...
class ActorNetwork_RNN(nn.Module):
    ...
    def forward(self, state):
        ...
        # get robot state feature embeddings
        ...
        gru_output, self._internal_state = self._gru(embedding, self._internal_state) # zero out internal state
        features1 = F.relu(self._h1(torch.hstack((gru_output,  state[:,state_index:]))))
        features2 = F.relu(self._h2(features1))
        a = self._h3(features2)
        return a
    
    # this feels a little "hacked", find better way to do this
    def reset_internal_state(self, reset):
        if reset:
            self._internal_state = None
