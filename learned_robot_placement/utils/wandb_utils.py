...
class wandbLogger:
    def __init__(self, exp_config, run_name, group_name, disabled=False):
        if disabled:
            ...
            mode=None
        wandb.init(project="rlmmbp_vision", entity="irosa-ias", group=group_name, id=run_name, config=exp_config,
                reinit=True, resume="allow", settings=wandb.Settings(start_method="fork"), mode=mode)

    def run_log_wandb(self,success_rate, J, R, E, avg_episode_length, q_loss, collisions):
        wandb.log({"success_rate": success_rate, "J": J, "R": R, "entropy": E, 
                                         "avg_episode_length": avg_episode_length, "q_loss": q_loss, "collisions":collisions})
